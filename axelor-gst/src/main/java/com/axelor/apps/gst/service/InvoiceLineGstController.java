...
public class InvoiceLineGstController extends InvoiceLineController {
	...
	public void compute(ActionRequest request, ActionResponse response) throws AxelorException {
		...
		if(invoiceLine.getTaxLine()!=null && invoiceLine.getTaxLine().getTax().getCode().contains("GST")) {
			...
			if(invoice.getAddress().getState()!=null && invoice.getCompany().getAddress().getState()!=null){
				...
				response.setFlash(I18n.get("Both the invoice address & company address should contain STATE information without which it is not possible to calculate the GST"));
			}
		}	
	}
}
